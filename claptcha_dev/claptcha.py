...
class Claptcha(object):
    ...
    def _writeText(self, image, text, pos):
        ...
        for c in text:
            ...
            c_draw.text((0, 0), c, font=self.font, fill=(int(random.random()*165), int(random.random()*150), int(random.random()*120), 255))
    ...
    def _drawLine(self, image):
        ...
        # Draw
        l_draw.line(((x1, y1), (x2, y2)), fill=(int(random.random()*255), int(random.random()*255), int(random.random()*255), 255), width=l_width)
    ...
    def _whiteNoise(self, size):
        ...
        if self.noise > 0.0012921569:  # 1./255.
            w, h = size

            pixelr = (lambda noise: round(255 * random.uniform(1-noise, 1)))
            pixelg = (lambda noise: round(255 * random.uniform(1-noise, 1)))
            pixelb = (lambda noise: round(255 * random.uniform(1-noise, 1)))

            n_image = Image.new('RGB', size, (0, 0, 0, 0))
            rnd_grid = map(lambda _: tuple([pixelr(self.noise), pixelg(self.noise), pixelb(self.noise)]),
                           [0] * w * h)
